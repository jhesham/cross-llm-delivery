...
1. Read this file top to bottom. Find the first unchecked `- [ ]` task whose phase is unblocked.
...
- [ ] Remove the stale **Gemini identity**: headline (`README.md:3-4` "Gemini 3.1 Pro"), prerequisites
...
- [ ] Single accurate prerequisites list: Python ≥3.11, git, Claude Code (state it explicitly — the
...
- [ ] Make the cross-platform path primary: lead skill generation with
      `python generator/build_skill.py --all` (portable); present `pwsh ./rebuild-skills.ps1` as the
      Windows convenience wrapper.
- [ ] Add the honest platform-support statement per **D2**.
- [ ] Remove references to non-existent "mirror repos tagged v<VERSION>" / `cross-llm-all` umbrella
...
- [ ] `pyproject.toml` `description` still says "cheap executor LLM (Gemini)" → reword provider-neutral
...
- [ ] Replace ALL `git@github.com:anthropic-ai/...` remotes with neutral `git@github.com:you/...`
      placeholders (looks like impersonating an official Anthropic project otherwise).
- [ ] Drop the `cross-llm-gemini` line (removed provider); ensure only antigravity/opencode/cursor
      (+ umbrella if kept) appear.
- **Done when:** file contains no `anthropic-ai` and no `gemini`.

### A4. Prune internal-only top-level docs
- [ ] `git rm HANDOFF-clean-install.md` (62KB one-off machine handoff; names DESKTOP-736AQ20).
- [ ] `git rm STATUS.md` (64KB private dev build-log; rac-agent references, resume protocol).
- [ ] `git rm docs/feedback-2026-07-02-asx-agent-build.md` (private asx-agent field report).
- [ ] `git rm docs/opencode-dispatch-bug-feedback.md` (internal bug note; content already fixed in code).
...
- [ ] Contains stderr logs, raw model-output captures, `C:\Users\Administrator` paths
...
- [ ] If prune: `git rm -r docs/superpowers/` (32 files, 535KB internal specs/plans; contains
...
- [ ] `tests/test_run_delivery.py:~245` comment "asx-agent field report 2026-07-02" → generic
      ("field report: --executor with an explicit model was silently swapped...").
- [ ] `tests/test_models.py:~132` comment "the RAC-thread failure" → describe the failure mode, not
      the private project.
...
- [ ] If squash: orphan branch (`git checkout --orphan public`), single initial commit of the
...
- [ ] Full-tree scans, all must be empty on the public branch:
...
- [ ] Secrets re-scan (should stay clean; keys appear only as env-var NAMES):
      `grep -rE "sk-[a-zA-Z0-9]{8}|pk-lf-[a-zA-Z0-9]|ghp_[a-zA-Z0-9]" <tracked>`.
- [ ] `python -m pytest -q` green; `python generator/build_skill.py --all` succeeds from the clean tree.
...
- [ ] Today a missing executor CLI surfaces as a raw `FileNotFoundError` at first dispatch. Add a
...
- [ ] Test: monkeypatched resolution failure → assert the friendly message + nonzero exit (no traceback).
...
- [ ] `INSTALL.md` is 100% PowerShell. Add a POSIX section (or parallel commands): clone,
...
- [ ] Verify every INSTALL.md claim against the current tree (it currently reads partly as notes
...
- [ ] Add a PLATFORM SUPPORT section to README (and echo in INSTALL.md): Windows = validated
...
- [ ] Recommend `--executor opencode:<model>` as the non-Windows default in the docs.
- **Done when:** a mac/Linux user is never surprised: the docs told them exactly what's proven.

### B4. Minimal CI
- [ ] Add `.github/workflows/ci.yml`: matrix `{ubuntu-latest, windows-latest}` × Python 3.11,
...
- [ ] If the ubuntu leg fails, fix portability issues it surfaces (this is the first-ever POSIX run —
...
- [ ] On a clean checkout (fresh temp clone of the public branch): follow README verbatim →
...
- [ ] Tag `v0.1.0` on the public branch; create a GitHub Release with a short feature summary.
- [ ] Add `CHANGELOG.md` (start at 0.1.0: provider-blind engine, 3 providers, deterministic judge,
...
- [ ] Cursor long-prompt headless dispatch defect (upstream CLI v2026.06.12; direct-node workaround
      shipped, revisit on cursor fix).
- [ ] Cost capture is opencode-only (`parse_opencode_usage` reads `step_finish.cost`); other
      providers show $0.00 (flat-rate or unreported).
- [ ] Telemetry `source` label: entry rung reports `default` even for an explicit `--executor` model
      (can't distinguish user-pinned vs default; `tag` only for per-slice tags).
- [ ] events.jsonl is per-build (a NEW build truncates the prior stream; per-run subdirs are the
      designed future fix).
- [ ] Evidence store (`~/.cld/validation-evidence.json`) is machine-local; "verified" statuses don't
      travel with the repo.
- **Done when:** file exists and each item names its workaround/status.

### C3. CONTRIBUTING.md stub
- [ ] How to run tests (`pip install -e ".[dev]"`, `pytest`), the engine/skill/generator layout
...
- [ ] This file is internal. Remove it from the public branch as the final pre-push commit
