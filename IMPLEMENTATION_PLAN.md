...
Implementation is in progress: **T01 through T14, T17 and M1–M4 are complete**. [T14 evidence](docs/plans/codex-support/T14-EVIDENCE.md) covers the offline flow, CLI and actual IDE skill invocation, Claude compatibility, and pinned Kimi K3 canary. [T17 child slices](docs/plans/codex-support/T17-SLICES.md) record its completed checkpoints. T18 is next. The checkboxes in the linked task files are the detailed source of progress; the tracker records task-level completion and evidence.
...
- 2026-09-23 — T12A complete: Kimi K3/OpenCode drafted the standalone Codex generator; the timed-out attempt was lead-reviewed and manually integrated. Nine corrected baseline cases remained red; 25 focused passed, local full suite passed, and [Windows/Ubuntu CI](https://github.com/jhesham/cross-llm-delivery/actions/runs/35845344191) passed tests and default generator smoke. Codex smoke passed locally for all three providers. Code/test head `8ef2a38`; [evidence](docs/plans/codex-support/T12A-EVIDENCE.md). Parent T12 stays open. Pause for explicit token confirmation before T12B.

- 2026-09-23 — T12B/T12 complete: pinned Kimi K3/OpenCode candidate passed collection and independent integration. Lead review moved Codex plugin default output to ignored `dist/plugins/` and fixed a pre-existing cancellation-test race found by Ubuntu CI. [Final Windows/Ubuntu CI](https://github.com/jhesham/cross-llm-delivery/actions/runs/35849789785) passed full tests, Claude/Codex generator smoke and Codex plugin packaging smoke; focused 38-case generator matrix and local full suite passed. Code/test head `3b1c0b9`; [evidence](docs/plans/codex-support/T12B-EVIDENCE.md). Stop for explicit token confirmation before T13.
- 2026-09-24 — T13A complete: Kimi K3/OpenCode candidate passed checked collection and independent integration; lead review corrected Python cache ownership and cleanup outcome semantics. A generated Codex bundle installed, ran from a nested cwd, updated and uninstalled in a disposable repo with spaces. [Final Windows/Ubuntu CI](https://github.com/jhesham/cross-llm-delivery/actions/runs/35891188570) passed full tests and all generator smoke. Code/test head `adbaf0c`; [evidence](docs/plans/codex-support/T13A-EVIDENCE.md). Parent T13 remains open; pause for token confirmation before T13B.
- 2026-09-24 — T13B/T13 complete: Kimi K3/OpenCode candidate passed committed red acceptance, single production collection and independent integration. Codex 0.155.1 app-server discovered the repo skill and all three local marketplace plugins from a nested disposable Git cwd; the vendored driver emitted a no-dispatch JSON dry run. Local focused/full tests and Windows/Ubuntu CI with all generator smoke passed. Code/test merge `f4b9551`; [evidence](docs/plans/codex-support/T13B-EVIDENCE.md). Pause for explicit token confirmation before T17.
- 2026-09-24 — T17A complete: installed wheel/sdist resources and `python -I -S` provider/CLI smoke are verified. Kimi K3/OpenCode's package-data candidate required manual lead application because CLD correctly protects `pyproject.toml` (`ae0556a`). Windows Python 3.11 CI then required a setuptools 77 build/dev minimum (`e979372`) for the existing SPDX license field. Local full suite and final Windows/Ubuntu CI passed; [evidence](docs/plans/codex-support/T17A-EVIDENCE.md). T17B owns six-bundle freshness, then T17C owns CI matrix/compatibility. Pause for token confirmation before T17B, so T14 remains gated on parent T17.
- 2026-09-24 — T17B complete: six fresh host/provider bundles passed isolated smoke and entrypoint checks; Codex plugin manifests/catalog passed offline checks. The tracked Claude plugins were refreshed from source (48 missing, 60 stale files) and a deterministic CI freshness gate now catches future drift. Local full suite and [Windows/Ubuntu CI](https://github.com/jhesham/cross-llm-delivery/actions/runs/35959871810) passed. Source `7997ecb`, `d09a2ab`; [evidence](docs/plans/codex-support/T17B-EVIDENCE.md). Pause before T17C; T14 remains gated on parent T17.
- 2026-09-24 — T17C/T17 complete: Python 3.11/3.14 × Windows/Ubuntu [CI](https://github.com/jhesham/cross-llm-delivery/actions/runs/35972684121) passed full offline tests, both-host generators and plugin gates. Plan/CLI-result/legacy-ledger fixtures, installed wheel and isolated core-only smoke passed; platform claims now state evidence levels and macOS is unverified. R07/A05 closed; R13 remains assigned to T18. [Evidence](docs/plans/codex-support/T17C-EVIDENCE.md); source `465b698`, timeout-fixture repair `5a5e68e`. Pause before T14.
- 2026-09-25 — T14/M4 complete: two-layer real-Git stop/resume, six host/provider contracts, Claude and Codex CLI discovery, one exact Kimi K3/OpenCode canary, and actual Codex VS Code skill selection/invocation passed. The IDE's vendored dry run resolved its installed script and reported `pending` with A/B layers without provider dispatch. [Evidence and captures](docs/plans/codex-support/T14-EVIDENCE.md). Stop for token confirmation before T18.
