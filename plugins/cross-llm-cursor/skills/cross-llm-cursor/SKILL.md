...
---
name: cross-llm-cursor
description: >-
  Route the bulk IMPLEMENTATION of a large software build to a cheap headless
  executor LLM (cursor:composer-2.5) while Claude acts as architect and
...
- **Default = the proven $0 flat-rate or low-cost workhorse** (`cursor:composer-2.5`). Enter selects it.
...
**Usage view:** run `run_delivery.py <plan> --usage` (or the `cross-llm-cursor-usage`
