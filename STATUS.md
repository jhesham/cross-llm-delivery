...
**Next task:** `T6.1` — Package as a Claude Code skill (skill-creator; `skill/` = SKILL.md + scripts wiring the orchestrator). **CLAUDE-DIRECT.** DoD: a `cross-llm-delivery` skill that, given a writing-plans plan, runs Phases 3-5; smoke-tested on a small real plan end-to-end (real Gemini dispatches through the assembled pipeline — the payoff). **Phase 5 FULLY COMPLETE (T5.1-T5.6): DAG fan-out, worktree isolation, integration gate, observability, behavioral Claude-judge eval. 21/22 done.** Then T6.2 (README/sharing), T6.3 (opt hook). FOLLOW-UP noted: GeminiExecutor.run could accept the feedback kwarg to actually use judge feedback on retry (currently plumbed via deliver_slice, ignored by the real executor's fixed signature).
...
| T6.1 Package as skill | 6 | ✅ done (Claude) | PH |
