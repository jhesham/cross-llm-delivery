...
def test_gate_passes_on_green_merged_tree():
    res = integration_gate(["A", "B"], run_full_suite=lambda: TestRun(0, "12 passed in 0.5s"))
...
def test_gate_fails_and_marks_rework_on_red():
    ...
    res = integration_gate(["A", "B"], run_full_suite=lambda: TestRun(1, out))
...
def test_gate_zero_collected_is_failure():
    res = integration_gate(["A"], run_full_suite=lambda: TestRun(5, "no tests ran in 0.01s"))
    assert res.passed is False
    assert res.rework_batch == ["A"]


def test_gate_captures_raw_output():
    res = integration_gate([], run_full_suite=lambda: TestRun(0, "3 passed in 0.1s"))
...
def test_gate_reuses_judge_parser():
    ...
    res = integration_gate(["X"], run_full_suite=lambda: TestRun(1, "5 passed, 2 failed in 0.3s"))
